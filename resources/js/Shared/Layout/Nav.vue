...
function submitSuggestion() {
    suggestionForm.post('/submitSuggestion', {
        onSuccess: () => {
            toggleSuggestionDialog()
            suggestionForm.reset()
            toast.success(__('You have submitted your suggestion successfully.'))
        },
    })
}
    ...
    <header class="fixed w-full bg-white">
        <nav class="mx-auto flex items-center justify-between px-6 py-3" aria-label="Global">
            ...
            <div class="hidden items-center md:flex md:gap-x-12">
                ...
                <div v-if="countCitiesWithoutAssignedCountry || countCitiesWithoutCoordinates"
                     class="flex items-center text-xs font-bold text-rose-500">
                    <InertiaLink v-if="countCitiesWithoutAssignedCountry" href="/admin/cities"
                    ...
                    <InertiaLink v-if="countCitiesWithoutCoordinates" href="/admin/cities?order=empty-coordinates"
        ...
        <div v-if="isAuthDialogOpened" class="fixed inset-0 z-50 flex items-center bg-black/50">
            <div ref="authDialog"
                    ...
                    <form class="space-y-5" @submit.prevent="login">
                        ...
                        <div class="relative">
                            ...
                            <input v-model="loginForm.password" :type="isPasswordVisible ? 'text' : 'password'"
                                   ...
                                   required
                            >
                            <button type="button" class="absolute bottom-3 right-2 md:bottom-2"
                ...
                <div v-else class="rounded-lg px-6 pb-8">
                    ...
                    <button :disabled="registerForm.processing" class="mt-6 text-xs font-light"
        ...
        <div class="fixed inset-0 z-50 flex items-center bg-black/50" v-if="isSuggestionDialogOpened">
            <div ref="suggestionDialog" class="mx-auto w-11/12 rounded-lg bg-white shadow-lg sm:w-3/4 md:w-2/3 lg:w-1/2 xl:w-1/3">
                ...
                <div class="rounded-lg px-6 pb-8">
                    <form class="space-y-5" @submit.prevent="submitSuggestion">
                        <div>
                            <label class="mb-1 block text-sm font-semibold text-gray-800">Name</label>
                            <input v-model="suggestionForm.name" type="text" class="w-full rounded-lg border-blumilk-200 py-3 md:p-2" required>
                        </div>
                        <div>
                            <label class="mb-1 block text-sm font-semibold text-gray-800">Email</label>
                            <input v-model="suggestionForm.email" type="email" class="w-full rounded-lg border-blumilk-200 py-3 md:p-2" required>
                        </div>
                        <div>
                            <label class="mb-1 block text-sm font-semibold text-gray-800">Suggestion</label>
                            <textarea v-model="suggestionForm.suggestion" class="w-full rounded-lg border-blumilk-200 py-3 md:p-2" rows="4" required></textarea>
                        </div>
                        <!-- Include any error messages here -->
                        <ErrorMessage :message="suggestionForm.errors.suggestionError"/>
                        <div class="flex w-full md:w-fit">
                            <button type="submit" @click=submitSuggestion class="w-full rounded-lg bg-blumilk-500 p-4 font-semibold text-white hover:bg-blumilk-600 md:py-2">
                                Submit Suggestion
                            </button>
                        </div>
                    </form>
                    ...
                    <div class="-my-6 divide-y divide-gray-500/10">
                        <div class="space-y-4 pt-6">
                            ...
                            <div v-if="countCitiesWithoutAssignedCountry || countCitiesWithoutCoordinates"
                                ...
                                <InertiaLink v-if="countCitiesWithoutCoordinates"
