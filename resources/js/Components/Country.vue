...
<template>
  <div class="m-2 flex flex-col rounded border bg-gray-50 p-2">
    ...
    <div v-if="isEditWindowOpened" class="mt-2 w-full space-y-2">
      <form class="flex w-1/2 flex-col space-y-2" @submit.prevent="updateCountry(country.id)">
        ...
        <div class="flex justify-between">
          <button v-show="isEditWindowOpened" type="submit" class="flex w-fit rounded border border-blue-500 bg-white px-5 py-2 text-white">
            <img v-show="isEditWindowOpened" width="18" src="https://img.icons8.com/ios/50/3382f6/save--v1.png" alt="">
          </button>

          <button v-show="isEditWindowOpened" class="border-light flex w-fit items-center space-x-1 rounded-full border border-gray-500 bg-gray-100 px-2" @click="openEditWindow">
    ...
    <div class="mt-2 flex space-x-2">
      <button v-if="!isEditWindowOpened" class="flex w-fit rounded bg-blue-500 px-5 py-2 text-white" @click="openEditWindow">
        <img width="18" src="https://img.icons8.com/ios-filled/50/FFFFFF/edit--v1.png" alt="">
      </button>

      <button v-if="!isEditWindowOpened" class="flex w-fit items-center rounded bg-rose-500 px-5 py-2 text-white" @click="destroyCountry(country.id)">
